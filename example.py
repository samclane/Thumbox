...
# New sprite designs
paddle_map = bytearray(
    [0b01111110, 0b11111111, 0b11111111, 0b01111110, 0b11111111, 0b01111110]
)
...
player2_sprite = thumby.Sprite(
    PADDLE_WIDTH,
    PADDLE_HEIGHT,
    paddle_map,
    graphics.display.width - PADDLE_WIDTH,
    player2_pos,
)
...
def update_paddles():
    ...
    if button.buttonU.pressed() and player1_pos > 0:
        ...
        button.buttonD.pressed()
        and player1_pos < graphics.display.height - PADDLE_HEIGHT
    ):
        player1_pos += PADDLE_SPEED
    ...
    if button.buttonA.pressed() and player2_pos > 0:
        ...
        button.buttonB.pressed()
        and player2_pos < graphics.display.height - PADDLE_HEIGHT
    ):
        player2_pos += PADDLE_SPEED
...
def update_ball():
    ...
    if ball_x <= PADDLE_WIDTH and (
        ...
        ball_dx = -ball_dx
    elif ball_x >= graphics.display.width - PADDLE_WIDTH - BALL_SIZE and (
...
def draw_scores():
    ...
    graphics.display.drawText(
        str(player2_score), (graphics.display.width * 3) // 4, 2, 1
    )
